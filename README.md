...
OIDC-only SSO plugin for [Better Auth](https://www.better-auth.com/) — runs anywhere with zero Node.js API dependencies.

## Why this exists

The official [`@better-auth/sso`](https://www.better-auth.com/docs/plugins/sso) plugin unconditionally imports `samlify` at module load, which requires Node.js-only APIs (`node:crypto`, `node:buffer`). This makes it impossible to use in edge runtimes, serverless environments, or any platform that doesn't provide full Node.js compatibility — even if you only need OIDC.

`better-auth-oidc` extracts the OIDC code paths into a standalone package. The SAML code and all Node.js dependencies are removed entirely.

## Runtime compatibility

| Runtime | `@better-auth/sso` | `better-auth-oidc` |
|---|---|---|
| Node.js | Yes | Yes |
| Convex | No | Yes |
...
```bash
# bun
bun add better-auth-oidc

# npm
npm install better-auth-oidc

# pnpm
pnpm add better-auth-oidc
```
...
import { oidcSso } from "better-auth-oidc";
...
import { oidcSsoClient } from "better-auth-oidc/client";
...
| `@better-auth/sso` | `better-auth-oidc` |
|---|---|
| `import { sso } from "@better-auth/sso"` | `import { oidcSso } from "better-auth-oidc"` |
| `import { ssoClient } from "@better-auth/sso/client"` | `import { oidcSsoClient } from "better-auth-oidc/client"` |
| `sso({ ... })` | `oidcSso({ ... })` |
